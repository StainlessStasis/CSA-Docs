...
in [message templates](../../config/message-templates/about) and [custom alert messages](custom_alert_messages.md).
...
| `{legendary} {legendary_unformatted}`           | Inserts the Pokemon's rarity (Legendary, Mythical, Ultra Beast, Paradox) if `showLegendary` is enabled in its respective [config](../../config/pokemon/about).   |
| `{shiny} {shiny_unformatted}`                   | Inserts a shiny message if the Pokemon is shiny and `alertShiny` is enabled in its respective [config](../../config/pokemon/about).                              |
| `{HA} {HA_unformatted}`                         | Inserts a hidden ability message if the Pokemon has a hidden ability and `alertHiddenAbility` is enabled in its respective [config](../../config/pokemon/about). |
